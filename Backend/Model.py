...
*** If the user is saying goodbye or wants to end the conversation like 'bye GuideLight.' respond with 'exit'.***
*** Respond with 'general (query)' if you can't decide the kind of query or if a query is asking to perform a task which is not mentioned above. ***
''' # PROMPT FOR THE AGENT (QUERY CLASSIFICATION) !!
# songs ,bye bye ,general kyu likh raha hai ,
...
def FirstLayerDMM(prompt: str = 'test'):
    message.append({"role": "User", "message": prompt})
    ...
    response =""
    for event in stream:
        if event.event_type == "text-generation": # adding response tokens to the cohere response....
            response += event.text
    ...
    if "(query)" in response:
        newresponse = FirstLayerDMM(prompt)  # looping until we get a proper response without (query)
        return newresponse
    else:
        return response
...
if __name__ == "__main__":
    while(True):
        print(FirstLayerDMM(input("User: ")))